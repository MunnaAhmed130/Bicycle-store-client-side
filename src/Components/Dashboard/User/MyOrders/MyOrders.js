...
const MyOrders = () => {
    ...
    useEffect(() => {
        fetch(`https://agile-island-88744.herokuapp.com/purchase/email?email=${user.email}`)
            .then(res => res.json())
    ...
    return (
        <div>
            ...
            <Row xs={1} md={2} lg={3} className="w-100 m-0">
                {
                    myOrders.map(myOrder => <MyOrder key={myOrder._id} myOrder={myOrder} handleDeleteOrder={handleDeleteOrder}></MyOrder>)
                }
            </Row>
        </div>
    );
